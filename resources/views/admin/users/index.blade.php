...
@section('content')
    ...
    <table class="table table-hover">
        ...
        @foreach($users as $user)
            <tr>
                <td>{{ $user->id }}</td>
                <td><img style="width: 80px; height: 85px; border-radius: 100%;" src="{{ $user->profile->image ? $user->profile->image : '/uploads/users/images/default.png'}}" alt="{{ $user->username ? $user->username : $user->name }} image"></td>
                <td>{{ $user->name }}</td>
                <td>{{ $user->email }}</td>
